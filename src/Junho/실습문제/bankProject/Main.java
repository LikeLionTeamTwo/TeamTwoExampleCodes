...
public class Main {
    public void start(Bank bank)  {
        ...
        System.out.println("=== " + bank.getName()+ " 은행 시스템 ===");
        int selectedNum = 0;
        while(selectedNum != 6){
            try{
                selectedNum = menu(sc);
                ...
                } else if (selectedNum == 3){ // 입금
                    Account account = findAccount(sc, bank);
                    deposit(sc,account);
                } else if( selectedNum == 4){ //출금
                    Account account = findAccount(sc, bank);
                    withdrawal(sc, account);
                } else if (selectedNum == 5) { //잔액 조회
                    Account account = findAccount(sc, bank);
                    printBalance(account);
                }
            } catch (Exception e){ // 종료
    ...
    public Account findAccount(Scanner sc, Bank bank){
        try {
            ...
            return  customer.findAccount(accountNumber);
    ...
    public void createAccount(Scanner sc, Bank bank)  {
        ...
        try{
            ...
            System.out.print("계좌 번호 입력: ");
            int accountNumber = sc.nextInt();
            Account account = new Account(accountNumber, customer.getId());
    ...
    public void addCustomer(Scanner sc, Bank bank){
        try{
            bank.addCustomer(this.createCustomer(sc));
        }catch (BankOperationException e){
            System.out.println(e);
        }
    }
